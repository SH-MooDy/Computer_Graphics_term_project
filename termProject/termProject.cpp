...
0.99f;  // 매 프레임 속도에 곱해지는 감속 계수 (1.0이면 마찰 없음)
const float SPIN_FRICTION = 0.97f;  // 스핀에 적용되는 별도 감속 계수
...
void initLighting() {
    ...
    glEnable(GL_LIGHT0);
    glEnable(
        GL_COLOR_MATERIAL);  // glColor 호출이 재질 색상에도 반영되도록 활성화
...
void initBalls() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        redBall.pos =
            Vec2(targetX + (i * 32.0f), targetY + (i % 2 == 0 ? 0 : 20.0f));
        redBall.r = 1.0f;
        redBall.g = 0.2f;
        redBall.b = 0.2f;
...
void applyHitSpinPhysics(Ball& ball, float angle_rad, float power,
    ...
    // 타격 각도에 따라 X/Y 방향 초기 속도 분해
    float vx = cos(angle_rad) * power;
    float vy = sin(angle_rad) * power;

    // 당점이 중심에서 벗어날수록 스핀 증가 - power에도 비례해 강하게 칠수록
    // 스핀도 강해짐
...
// 스핀 효과 적용 -> 마찰 감속 -> 위치 갱신 -> 벽 반사 -> 공 간 충돌 처리 순으로
// 실행
void updatePhysics(int value) {
    ...
    for (size_t i = 0; i < balls.size(); i++) {
        ...
        if (speed > 0.1f) {
            ...
            // 사이드 스핀: 이동 방향의 수직 벡터(-dy, dx)로 횡력 적용 -> 곡선 궤적
            // 생성
        ...
        // 벽 충돌 - 법선 방향 속도 반전, 사이드 스핀도 부분 반전
...
void drawHitPointIndicator(float screenX, float screenY, float offsetX,
    float offsetY) {
    glDisable(GL_LIGHTING);
    glDisable(GL_DEPTH_TEST);
    ...
    glLineWidth(1.0f);
    glBegin(GL_LINES);
    glVertex2f(cx - displayR, cy);
    glVertex2f(cx + displayR, cy);
    glVertex2f(cx, cy - displayR);
    glVertex2f(cx, cy + displayR);
    glEnd();
...
// 당구대 -> 3D 구조물(3D 시점 한정) -> 조준선 -> 공 -> 큐대 -> HUD 순서로
// 렌더링
void display() {
    ...
    if (!isTopView)
        gluLookAt(camEyeX, camEyeY, camEyeZ, camCenterX, camCenterY, camCenterZ,
            0.0f, 0.0f, 1.0f);
    ...
    if (!isTopView) {
        ...
        // 테이블 판 본체
        glPushMatrix();
        glTranslatef(400.0f, 200.0f, -11.0f);
        glScalef(800.0f, 400.0f, 20.0f);
        ...
        glPushMatrix();
        glTranslatef(400.0f, 415.0f, 5.0f);
        glScalef(860.0f, 30.0f, 20.0f);
        glutSolidCube(1.0f);
        glPopMatrix();
        glPushMatrix();
        glTranslatef(400.0f, -15.0f, 5.0f);
        glScalef(860.0f, 30.0f, 20.0f);
        glutSolidCube(1.0f);
        glPopMatrix();
        glPushMatrix();
        glTranslatef(-15.0f, 200.0f, 5.0f);
        glScalef(30.0f, 400.0f, 20.0f);
        glutSolidCube(1.0f);
        glPopMatrix();
        glPushMatrix();
        glTranslatef(815.0f, 200.0f, 5.0f);
        glScalef(30.0f, 400.0f, 20.0f);
        glutSolidCube(1.0f);
        glPopMatrix();
        ...
        float legH = 100.0f;
        glPushMatrix();
        glTranslatef(20.0f, 20.0f, legZ);
        glScalef(30.0f, 30.0f, legH);
        glutSolidCube(1.0f);
        glPopMatrix();
        glPushMatrix();
        glTranslatef(780.0f, 20.0f, legZ);
        glScalef(30.0f, 30.0f, legH);
        glutSolidCube(1.0f);
        glPopMatrix();
        glPushMatrix();
        glTranslatef(20.0f, 380.0f, legZ);
        glScalef(30.0f, 30.0f, legH);
        glutSolidCube(1.0f);
        glPopMatrix();
        glPushMatrix();
        glTranslatef(780.0f, 380.0f, legZ);
        glScalef(30.0f, 30.0f, legH);
        glutSolidCube(1.0f);
        glPopMatrix();
    }
    ...
    if (isCueVisible) {
        ...
        glBegin(GL_LINES);
        glVertex3f(balls[0].pos.x, balls[0].pos.y,
            isTopView ? 0.0f : balls[0].radius);
        glVertex3f(balls[0].pos.x + cos(rad) * 1000.0f,
            balls[0].pos.y + sin(rad) * 1000.0f,
            isTopView ? 0.0f : balls[0].radius);
        glEnd();
    }
    ...
    // 공 렌더링
    // 공마다 glRotatef로 이동 방향 기준 자전을 적용하고, 납작한 점 6개를 표면에
    // 배치
    for (size_t i = 0; i < balls.size(); i++) {
        glPushMatrix();
        glTranslatef(balls[i].pos.x, balls[i].pos.y,
            isTopView ? 0.0f : balls[i].radius);

        // 자전 적용: 이동 방향의 수직 벡터(-lastDir.y, lastDir.x, 0)를 축으로
        // rotAngle만큼 회전 이렇게 하면 공이 구르는 방향으로 자연스럽게 자전
        ...
        // 해당 축 방향으로 배치되는 마커는 그 축을 0.05배로 납작하게 눌러 스티커
        // 형태 구현
        float dotParams[6][6] = {
    ...
    if (isCueVisible) {
        glPushMatrix();
        glTranslatef(balls[0].pos.x, balls[0].pos.y,
            isTopView ? 0.0f : balls[0].radius);
    ...
    if (isCueVisible) {
        ...
        drawText2D(10, h - 38,
            "A/D: Angle  W/S: Power  J/L: Side Spin  I/K: Top/Back Spin");
...
void keyboard(unsigned char key, int x, int y) {
    ...
    if (isCueVisible && !isStriking) {
        ...
        case 'w':
        case 'W':
            if (cuePower < 30.0f) cuePower += 1.5f;
            break;  // 파워 증가
        case 's':
        case 'S':
            if (cuePower > 2.0f) cuePower -= 1.5f;
            break;  // 파워 감소
        case 'j':
        case 'J':
            if (hitOffsetX > -1.0f) hitOffsetX -= 0.1f;
            break;  // 당점 좌로
        case 'l':
        case 'L':
            if (hitOffsetX < 1.0f) hitOffsetX += 0.1f;
        ...
        case 'i':
        case 'I':
            if (hitOffsetY < 1.0f) hitOffsetY += 0.1f;
            break;  // 당점 위로 (탑스핀)
        case 'k':
        case 'K':
            if (hitOffsetY > -1.0f) hitOffsetY -= 0.1f;
...
void specialKeys(int key, int x, int y) {
    ...
    if (!isTopView) {
        ...
        float currentAngle = std::atan2(dy, dx);      // 현재 카메라의 수평 각도
